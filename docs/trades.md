...
`status='open'` 且非逆回购的持仓列表，供前端持仓市值/分布饼图使用：

| 字段 | 说明 |
|------|------|
| `symbol` / `name` | 股票代码与名称 |
| `entry_price` / `quantity` | 买入价与数量 |
| `take_profit` / `stop_loss` / `breakeven` | 风控价（可空） |
| `model_id` | 关联量化模型 ID（`null`=无） |

浮盈亏与持仓金额由前端结合 `/api/quotes` 现价计算。持仓分布展示为：**左圆环 60%（按股票市值聚合，同股多笔合并）+ 右侧图例 40%（名称、模型简称 `a60m|b30m`、占比同色）+ 下方 slim 表格（按量化模型汇总笔数/金额/占比）**；点击图例或表格行可跳转 K 线或筛选持仓列表。模型简称取模型名按空白切分后的首段。
...
| `entry_reason` | 该侧笔数最前一笔买入理由（列表按笔数第1→第N 合并非空理由） |
| `exit_reason` | 该侧笔数最前一笔卖出理由（有卖出时；列表同上） |
...
交易记录可填可选的止盈 / 保本 / 止损价（弹窗「风控价格」）。**单笔与批次均可填**（规则相同，批次写在父行、相对净持仓加权均价）；逆回购无此面板。**须三个都填或都空**，且 **止盈 > 保本 > 止损**；空串表示清空。旧记录不完整或顺序不对时仍可读，列表该列文字标黄；改风控字段时须符合新规则。填齐三个价的 **open 持仓**，在用户被授权监控后，由 `visual/monitor.py` 在交易时段后台跟踪。持仓行在现价刷新时展示「止盈/保本/止损 + 相对现价的差百分比」（正红负绿）。

- **谁会被监控**：管理员恒开；普通用户需 `monitor_enabled=1`。未填风控价的持仓（含批次）不进监控列表；`reverse_repo` 不参与价位监控。
