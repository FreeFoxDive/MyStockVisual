...
TODAY = NOW.date()
STAMP = NOW.replace(tzinfo=timezone(timedelta(hours=8))).timestamp()
...
def _af_quote(price=11.5, volume=5000, high=12.0, low=10.5, ts=None):
    ...
    return {
        ...
        "timestamp": STAMP if ts is None else ts,
        "name": "测试标的",
    }
...
class _Harness(unittest.TestCase):
    ...
    def setUp(self):
        clock = mock.patch("market_hours.now", return_value=NOW)
        clock.start()
...
class TestGuardsHoldWithReusedSnapshot(_Harness):
    ...
    def test_stale_snapshot_timestamp_rejected(self):
        ...
        self._quote_for = lambda s: _af_quote(ts=STAMP - 86400)
        df, _, _ = self._chart(quote_fresh=False)
