# -*- coding: utf-8 -*-
"""visual/server.py 质押数据缓存与定时更新的单元测试。
...
# visual/ 不是包（无 __init__.py），把其目录加入 sys.path 后直接 import server
...
class PledgeTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._orig_script_dir = server.SCRIPT_DIR
        server.SCRIPT_DIR = Path(self._tmp.name)
        # 复位模块级质押全局状态
        server._pledge_cache = None
        server._pledge_ts = 0
        server._pledge_date = None
        server._refreshing_pledge = False

    def tearDown(self):
        server.SCRIPT_DIR = self._orig_script_dir
        server._pledge_cache = None
        server._pledge_ts = 0
        server._pledge_date = None
        server._refreshing_pledge = False
    ...
    def _wait_refresh(self, timeout=5.0):
        ...
        while server._refreshing_pledge and time.time() < deadline:
            time.sleep(0.01)
        self.assertFalse(server._refreshing_pledge, "后台刷新超时未完成")

    # ---- 文件名 / 磁盘缓存 ----

    def test_file_path_self_descriptive(self):
        fp = server._pledge_file_path("20260818")
    ...
    def test_disk_roundtrip(self):
        pledge = _mk_pledge()
        server._pledge_to_disk("20260818", pledge)
        date, got, ts = server._pledge_from_disk()
        self.assertEqual(date, "20260818")
        self.assertEqual(got, pledge)
        self.assertGreater(ts, 0)
        # 确实是 gzip 文件
        self.assertTrue(server._pledge_file_path("20260818").exists())

    def test_disk_retention_keeps_latest(self):
        # 写 8 天, 应只保留最近 7 天, 删最旧 1 天
        dates = [f"202608{i:02d}" for i in range(11, 19)]  # 11..18
        for i, d in enumerate(dates):
            server._pledge_to_disk(d, {"000001": {"ratio": float(i)}})
        files = list((server.SCRIPT_DIR / ".cache").glob("pledge_ratio_*.json.gz"))
        self.assertEqual(len(files), server.PLEDGE_KEEP_FILES)
        self.assertTrue(server._pledge_file_path("20260818").exists())
        self.assertFalse(server._pledge_file_path("20260811").exists())

    def test_disk_reads_latest_by_date(self):
        # 多个文件时应取日期最新 (字符串序 = 日期序)
        server._pledge_to_disk("20260816", {"000001": {"ratio": 1.0}})
        server._pledge_to_disk("20260818", {"000001": {"ratio": 2.0}})
        date, got, _ = server._pledge_from_disk()
        self.assertEqual(date, "20260818")
    ...
    def test_fetch_parses_and_backtracks(self):
        ...
        with mock.patch.dict(sys.modules, {"akshare": fake}):
            got_date, pledge = server._fetch_pledge()
    ...
    def test_load_first_run_fetches_and_persists(self):
        pledge = _mk_pledge()
        with mock.patch.object(server, "_fetch_pledge",
                               return_value=("20260818", pledge)):
            result = server._load_pledge()
        self.assertEqual(result, pledge)
        self.assertEqual(server._pledge_cache, pledge)
        self.assertEqual(server._pledge_date, "20260818")
        # 同时落盘
        date, got, _ = server._pledge_from_disk()
        self.assertEqual(date, "20260818")
        self.assertEqual(got, pledge)
    ...
    def test_load_fresh_memory_no_fetch(self):
        pledge = _mk_pledge()
        server._pledge_cache = pledge
        server._pledge_ts = time.time()
        server._pledge_date = "20260818"
        with mock.patch.object(server, "_fetch_pledge") as m, \
             mock.patch.object(server, "_refresh_pledge_async") as r:
            result = server._load_pledge()
        self.assertEqual(result, pledge)
        m.assert_not_called()
        r.assert_not_called()
    ...
    def test_load_stale_memory_returns_old_and_refreshes(self):
        pledge = _mk_pledge()
        server._pledge_cache = pledge
        server._pledge_ts = time.time() - 100000  # 过期
        server._pledge_date = "20260817"
        with mock.patch.object(server, "_refresh_pledge_async") as r:
            result = server._load_pledge()
    ...
    def test_load_disk_fallback(self):
        pledge = _mk_pledge()
        server._pledge_to_disk("20260818", pledge)  # 内存空, 磁盘有(新鲜)
        with mock.patch.object(server, "_fetch_pledge") as m, \
             mock.patch.object(server, "_refresh_pledge_async") as r:
            result = server._load_pledge()
        self.assertEqual(result, pledge)
    ...
    def test_load_fetch_failure_returns_empty(self):
        with mock.patch.object(server, "_fetch_pledge", return_value=(None, None)):
            result = server._load_pledge()
        self.assertEqual(result, {})
        self.assertEqual(server._pledge_cache, {})

    # ---- _refresh_pledge_async 失败处理 ----

    def test_refresh_failure_keeps_old_cache(self):
        old = _mk_pledge(ratio=9.9)
        server._pledge_cache = old
        server._pledge_ts = time.time()
        server._pledge_date = "20260817"
        with mock.patch.object(server, "_fetch_pledge", return_value=(None, None)):
            server._refresh_pledge_async()
            self._wait_refresh()
        self.assertEqual(server._pledge_cache, old)      # 旧缓存保留
        self.assertEqual(server._pledge_date, "20260817")

    def test_refresh_success_updates_cache_and_disk(self):
        old = _mk_pledge(ratio=9.9)
        server._pledge_cache = old
        server._pledge_ts = time.time() - 100000
        server._pledge_date = "20260817"
        ...
        with mock.patch.object(server, "_fetch_pledge",
            ...
            server._refresh_pledge_async()
            self._wait_refresh()
        self.assertEqual(server._pledge_cache, new)
        self.assertEqual(server._pledge_date, "20260818")
        date, got, _ = server._pledge_from_disk()
        self.assertEqual(date, "20260818")
    ...
    def test_refresh_same_date_skips_write(self):
        ...
        with mock.patch.object(server, "_fetch_pledge",
            ...
            server._refresh_pledge_async()
            self._wait_refresh()
        self.assertEqual(server._pledge_cache, old)
    ...
    def test_schedule_delay_before_1530(self):
        ...
        self.assertEqual(server._next_schedule_delay(now), 5.5 * 3600)

    def test_schedule_delay_after_1530(self):
        from datetime import datetime
        now = datetime(2026, 8, 18, 16, 0, 0)
        self.assertEqual(server._next_schedule_delay(now), 23.5 * 3600)

    def test_schedule_delay_exactly_1530(self):
        from datetime import datetime
        now = datetime(2026, 8, 18, 15, 30, 0)
        self.assertEqual(server._next_schedule_delay(now), 24 * 3600)
